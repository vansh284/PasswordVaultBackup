...
public class AuthValidator
{
        ...
        bool requireVerified = true)
    {
        ...
        // 3. Monotonic nonce replay protection: nonce must be strictly greater than LastNonce
        ...
        // 4. Decode base64 payload bytes and signature
        byte[] rawPayloadBytes;
        byte[] signatureBytes;
        try
        {
        ...
        // 5. Build canonical message: UTF8(email) || 0x00 || rawPayloadBytes || 0x00 || UTF8(nonce)
        byte[] canonicalMessage = SigningService.BuildSignedMessage(email, rawPayloadBytes, envelope.Nonce);

        // 6. Verify signature against stored ECDSA public key
        try
        {
